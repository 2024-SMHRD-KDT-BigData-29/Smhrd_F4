...
class PowerDataCreate(BaseModel):
    ...
    p_data: Optional[datetime] = None # 측정 시간 (tb_power_data.p_data에 해당)
...
class PowerDataRecordResponse(BaseModel):
    p_idx: int
    he_idx: int
    p_data: datetime # SQLAlchemy 모델의 p_data 컬럼 (타임스탬프)
...
class HourlyPowerConsumptionResponse(BaseModel):
    timestamp: datetime  # PowerDataORM.p_data 컬럼 값이 여기에 매핑됩니다.
